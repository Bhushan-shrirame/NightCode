...
<img src="./Demo_image.png" alt="NightCode Demo" width="100%" />

</div>

## Architecture

NightCode is built on a clean monorepo architecture separating the terminal-based client from the cloud API and database orchestration layers:

<img src="./architecture.svg" alt="NightCode Architecture" width="100%" />

### Core Components

1. **CLI Client (`packages/cli`)**:
   - **Terminal UI**: Built with **React** and **OpenTUI** (a custom terminal renderer reconciler), handling interactive grids, textareas, status bars, and overlays.
   - **Local Tool Execution**: When the AI model requests a tool call (such as reading a file, writing a file, or running a shell command), the CLI intercepts the request and runs it locally on your computer, streaming back the output.
2. **Server API (`packages/server`)**:
   - **Router**: Powered by **Hono** and **Bun** to orchestrate requests and stream SSE updates.
   - **Middleware**: Validates session authentication via **Clerk** and verifies available credit limits via **Polar**.
   - **Billing & Token Usage**: Converts model tokens (input/output counts) into billable credits and reports them to Polar for metered charging.
3. **Database (`packages/database`)**:
   - Configured with **Prisma ORM** connecting to PostgreSQL.
   - Stores session history and serialized `UIMessage` structures.
4. **Shared Package (`packages/shared`)**:
   - A light dependency containing shared TypeScript interfaces, Zod schemas, model registries, and tool parameter definitions.
