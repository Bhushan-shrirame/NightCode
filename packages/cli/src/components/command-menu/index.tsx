...
const COMMMAND_COL_WIDTH = Math.max(...COMMANDS.map((cmd) => cmd.name.length)) + 4
...
export function CommandMenu( {query, selectedIndex, scrollRef, onSelect, onExecute }: CommandMenuProps) {
    ...
    if(filtered.length === 0){
        return (
            <box paddingX={1}>
                <text attributes={TextAttributes.DIM}>no mathcing commands
                </text>
            </box>
        )
    }
        ...
        <scrollbox ref={scrollRef} height={visibleHeight}>
            {filtered.map((cmd, i) => {
                ...
                return(
                    ...
                    >
                        <box width={COMMMAND_COL_WIDTH} flexShrink={0}>
